...
export class OrdersGrid extends Component {
  ...
  #createOrderDetailsHTML(order) {
    ...
    order.products.forEach((productDetails) => {
      ...
      const deliveryDateMessage =
        Date.now() < productDetails.estimatedDeliveryTimeMs
          ? `Arriving on: ${DateUtils.formatDateMonth(
              productDetails.estimatedDeliveryTimeMs
            )}`
          : `Delivered on: ${DateUtils.formatDateMonth(
              productDetails.estimatedDeliveryTimeMs
            )}`;
  ...
  #trackPackage(event) {
    ...
    WindowUtils.setHref(
      `tracking.html?orderId=${orderId}&cartItemId=${cartItemId}`
    );
  }
}
