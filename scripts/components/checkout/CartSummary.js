...
export class CartSummary extends ComponentV2 {
  events = {
    ...
    "click .js-update-quantity-link": (event) =>
      this.#toggleQuantityEditor(event),
    "keyup .js-new-quantity-input": (event) => this.#handleQuantityInput(event),
    "click .js-save-quantity-link": (event) => this.#handleSaveLinkClick(event),
    "click .js-delete-quantity-link": (event) =>
  ...
  #selectDeliveryOption(event) {
    ...
    const radioInput = deliveryOptionElem.querySelector(
    ...
    const deliveryOptionId = deliveryOptionElem.getAttribute(
    ...
    cartItemElem.querySelector(".js-delivery-date").textContent =
      DateUtils.formatDateWeekday(newDeliveryDate);

    this.#updatePaymentDetailsAndHeader();
  }

  #toggleQuantityEditor(event) {
    const quantityContainer = event.currentTarget.closest(
  ...
  #cancelUpdateQuantity(quantityContainer) {
    const inputElement = quantityContainer.querySelector(
      ".js-new-quantity-input"
    );
    quantityContainer.classList.remove("is-updating-quantity");

    const oldQuantity =
      quantityContainer.querySelector(".js-quantity-label").textContent;
