...
export class PaymentSummary extends Component {
  element;
  events = {
    "click .js-paypal-toggle": (event) => this.#togglePaypal(event),
  };
  ...
  constructor(selector) {
    ...
    this.#usePaypal =
      localStorage.getItem("exercises-amazon-use-paypal") === "true";
  }
  ...
  render() {
    this.element.innerHTML = `
      ...
      <div class="js-payment-buttons-container ${
        this.#usePaypal && "use-paypal"
      }"
    ...
    this.events["click .js-place-order-button"] = (event) =>
      this.#performCheckout(event);
  }

  refreshPaymentDetails() {
    const { productCostCents, shippingCostCents, taxCents, totalCents } =
      cart.calculateCosts();
