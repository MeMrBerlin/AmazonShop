...
export class ProductsGrid extends ComponentV2 {
  events = {
    "click .js-add-to-cart-button": (event) => this.#addToCart(event),
  ...
  render() {
    ...
    searchResults.forEach((product) => {
      ...
      productsGridHTML += `
        <div class="js-product-container product-container" data-product-id="${
          product.id
        }"
  ...
  #addToCart(event) {
    // Add product to cart.
    const productContainer = event.currentTarget.closest(
    ...
    const successMessageElement = productContainer.querySelector(
      ".js-added-to-cart-message"
    );
  ...
  #selectVariation(event) {
    ...
    const variationsContainer = button.closest(
      ".js-variation-options-container"
    );

    // Switch the selected variation.
    const previousButton = variationsContainer.querySelector(
      ".js-selected-variation"
    );
  ...
  #getSelectedVariation(productContainer) {
    ...
    productContainer
      .querySelectorAll(".js-selected-variation")
      .forEach((button) => {
